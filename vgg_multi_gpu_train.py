...
def train():
  ...
  with tf.Graph().as_default(), tf.device('/cpu:0'):
    ...
    WEIGHTS_DECAY = False
    if WEIGHTS_DECAY:
      num_batches_per_epoch = (vggnet.NUM_EXAMPLES_PER_EPOCH_FOR_TRAIN /
                               FLAGS.batch_size)
      decay_steps = int(num_batches_per_epoch * vggnet.NUM_EPOCHS_PER_DECAY)

      # Decay the learning rate exponentially based on the number of steps.
      lr = tf.train.exponential_decay(vggnet.INITIAL_LEARNING_RATE,
                                      global_step,
                                      decay_steps,
                                      vggnet.LEARNING_RATE_DECAY_FACTOR,
                                      staircase=True)
    ...
    with tf.variable_scope(tf.get_variable_scope()) as scope:
      for i in xrange(FLAGS.num_gpus):
        with tf.device('/gpu:%d' % i):
          with tf.name_scope('%s_%d' % (vggnet.TOWER_NAME, i)) as scope:
